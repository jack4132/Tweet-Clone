...
def register():
    try:
        ...
        if(len(list(filter(lambda x: x["email"] == email, users))) == 1):
            return jsonify({"error": "Invalid form"})
        # Email validation check (a valid email, according to my RegEx, is one that has 5 or more alphanumberic..\n
        # characters including . and _, an @, 3 or more alphanumeric characters, a . and 2-4 more alphanumberic..\n
        # characters)
